  ...
  .action(async (env: string | undefined, opts: { json?: boolean }) => {
    try {
      ...
      if (opts.json && env) {
        ...
        const wt = wtList.find((w) => w.branch === env || w.branch.endsWith(`/${env}`));
        if (!wt) {
          console.log(JSON.stringify({ ok: false, error: `no worktree found for: ${env}` }));
          process.exit(1);
        }
      ...
      if (ghWarning) process.stderr.write(chalk.yellow(`\n  ⚠  grove: ${ghWarning}\n\n`));
...
program
  .command('open [branch]')
  .description('Open worktree in editor (respects grove config, $VISUAL, $EDITOR)')
  .action(async (branch?: string) => {
    const { openInEditor, editorDisplayName, resolveEditor } = await import('./tui/editor.js');
    try {
      const repoPath = await detectRepoRoot();
      ...
      if (!editor) {
        console.error('No editor found — set $VISUAL, $EDITOR, or add "editor" to .grove/config.json');
        process.exit(1);
      }
...
program
  ...
  .option('--new', 'Create a new branch (use --base to set the starting point, defaults to main)')
  .option('--base <branch>', 'Base branch for --new (default: main)')
  .option('--json', 'Output machine-readable JSON')
  .action(async (target: string, opts: { new?: boolean; base?: string; json?: boolean }) => {
    ...
    try {
      ...
      const worktreeRoot = resolveWorktreeRoot(repoPath, repoGroveConfig?.worktrees?.root);
      ...
      if (/^\d+$/.test(target)) {
        ...
        const { stdout } = await execa('gh', ['pr', 'view', target, '--json', 'headRefName', '--jq', '.headRefName']);
        branch = stdout.trim();
        if (!opts.json) console.log(`  → branch: ${branch}`);
      }

      const worktreePath = pathMod.join(worktreeRoot, branch.replace(/\//g, '-'));
      ...
      if (!existsSync(worktreePath)) {
        ...
        if (opts.new) {
          ...
          await execa('git', ['worktree', 'add', '-b', branch, worktreePath, base], { cwd: repoPath });
          ...
          await writeFile(pathMod.join(groveMetaDir, 'meta.json'), JSON.stringify({ baseBranch: base }, null, 2));
        } else {
          await execa('git', ['fetch', 'origin', branch], { cwd: repoPath });
          await execa('git', ['worktree', 'add', worktreePath, branch], { cwd: repoPath });
        }
      } else if (!opts.json) {
      ...
      const groveConfig = await loadGroveConfig(worktreePath) ?? await loadGroveConfig(repoPath);
      if (!existsSync(envAgentPath)) {
        if (groveConfig) {
          if (!opts.json) console.log('Generating .env.worktree from grove config…');
      ...
      if (sharedInfo) {
        ...
        if (sharedState !== 'running') {
          if (!sharedInfo.exists) {
            if (!opts.json) console.log(chalk.yellow(`  warning: shared stack configured (${sharedInfo.projectName}) but ${sharedInfo.composeFile} not found`));
          } else {
            if (!opts.json) console.log(`Starting shared stack (${sharedInfo.projectName})…`);
            await sharedUp(sharedInfo, repoPath);
            if (!opts.json) console.log(`✓ Shared stack running`);
          }
        } else if (!opts.json) {
          console.log(chalk.gray(`  shared stack: ${sharedInfo.projectName} already running`));
        }
      }
...
program
  ...
  .action(async (env: string) => {
    try {
      ...
      const wt = worktrees.find((w) => w.branch === env || w.branch.endsWith(`/${env}`));
      if (!wt) {
...
docker
  ...
  .action(async (branch?: string) => {
    ...
    try {
      ...
      const { worktrees } = await loadWorktrees(repoPath);
      const wt = branch
        ? worktrees.find((w) => w.branch === branch || w.branch.endsWith(`/${branch}`))
        : worktrees[0];
      if (!wt) { console.error('No worktree found'); process.exit(1); }
      if (!wt.docker) { console.error(`No .env.worktree found in ${wt.path}`); process.exit(1); }
      const { projectName } = wt.docker;
      console.log(`Starting ${projectName}…`);
      await execa('docker', ['compose', '-p', projectName, '--env-file', '.env.worktree', 'up', '-d', '--build'], { cwd: wt.path });
...
docker
  ...
  .action(async (branch?: string) => {
    ...
    try {
      ...
      await execa('docker', ['compose', '-p', projectName, '--env-file', '.env.worktree', 'down'], { cwd: wt.path });
...
docker
  ...
  .description('Tear down docker compose stack and volumes (destructive — prompts for confirmation)')
  .action(async (branch?: string) => {
    const { execa } = await import('execa');
    ...
    try {
      ...
      await execa('docker', ['compose', '-p', projectName, '--env-file', '.env.worktree', 'down', '-v'], { cwd: wt.path });
...
shared
  .command('up')
  .description(`Start the shared stack (default file: ${DEFAULT_SHARED_COMPOSE_FILE})`)
  .action(async () => {
    try {
      const repoPath = await detectRepoRoot();
      ...
      if (!info) {
        console.error('No shared stack configured. Add naming.sharedProject to .grove/config.json.');
        process.exit(1);
      }
      if (!info.exists) {
      ...
      if (state === 'running') {
        console.log(chalk.gray(`Shared stack already running (${info.projectName})`));
        return;
      }
  ...
  .action(async () => {
    try {
      ...
      if (state === 'not started' || state === 'stopped') {
        console.log(chalk.gray(`Shared stack is not running (${info.projectName})`));
        return;
      }
...
program
  ...
  .option('--delete-branch', 'Also delete the git branch after removing the worktree')
  .action(async (branch: string, opts: { yes?: boolean; deleteBranch?: boolean }) => {
    const { execa } = await import('execa');
    const readline = await import('readline');
    ...
    try {
      const repoPath = await detectRepoRoot();
      const { worktrees } = await loadWorktrees(repoPath);
      const wt = worktrees.find((w) => w.branch === branch || w.branch.endsWith(`/${branch}`));
      if (!wt) {
      ...
      if (wt.docker && wt.docker.state !== 'not started' && wt.docker.state !== 'stopped') {
        console.log(`Stopping docker stack ${wt.docker.projectName}…`);
        try {
          await execa(
            'docker',
            ['compose', '-p', wt.docker.projectName, '--env-file', '.env.worktree', 'down'],
            { cwd: wt.path },
          );
      ...
      await execa('git', ['worktree', 'remove', '--force', wt.path], { cwd: repoPath });
...
program
  ...
  .action(async () => {
    try {
      ...
      const rootSource =
        process.env.GROVE_WORKTREE_ROOT ? '$GROVE_WORKTREE_ROOT' :
        groveConfig?.worktrees?.root    ? '.grove/config.json'    :
                                          'default';

      console.log(`\n${chalk.bold.cyan('grove')} ${chalk.gray('· repo info')}\n`);
      console.log(`${chalk.gray('repo')}          ${repoPath}`);
      console.log(`${chalk.gray('worktree root')} ${worktreeRoot}  ${chalk.gray(`(${rootSource})`)}`);
      console.log(`${chalk.gray('grove config')}  ${groveConfig ? chalk.green('found') : chalk.gray('none')}`);
      if (groveConfig) {
        console.log(`${chalk.gray('  project')}     ${groveConfig.project}`);
        console.log(`${chalk.gray('  editor')}      ${groveConfig.editor ?? chalk.gray('(not set)')}`);
      }
      console.log();
      console.log(chalk.gray('To override worktree root:'));
      console.log(chalk.gray('  env:    ') + 'GROVE_WORKTREE_ROOT=/your/path grove start ...');
      console.log(chalk.gray('  config: ') + 'set "worktrees": { "root": "/your/path" } in .grove/config.json');
      console.log();
    } catch (err) {
      console.error((err as Error).message);
      process.exit(1);
...
program
  ...
  .action(async (opts: { preset?: string; dryRun?: boolean; yes?: boolean; reset?: boolean }) => {
    try {
      const repoPath = await detectRepoRoot();
