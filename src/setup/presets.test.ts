...
describe('recommendPreset', () => {
  test('recommends docker when hasDockerCompose is true', () => {
    expect(recommendPreset(makeDetection({ hasDockerCompose: true }))).toBe('docker');
  });
  ...
  test('recommends node when hasPackageJson and no docker or vite', () => {
    expect(recommendPreset(makeDetection({ hasPackageJson: true }))).toBe('node');
  });
  ...
  test('docker takes priority over vite framework', () => {
    expect(recommendPreset(makeDetection({ hasDockerCompose: true, framework: 'vite' }))).toBe('docker');
  });

  test('docker takes priority over packageJson', () => {
    expect(recommendPreset(makeDetection({ hasDockerCompose: true, hasPackageJson: true }))).toBe('docker');
  });

  test('node wins over null framework when packageJson present', () => {
    expect(recommendPreset(makeDetection({ hasPackageJson: true, framework: null }))).toBe('node');
  });
});
...
describe('presets.docker.generate', () => {
  test('creates docker-compose provider for each detected app service', () => {
    ...
    expect(config.providers.web).toEqual({ type: 'docker-compose', service: 'web' });
    expect(config.providers.api).toEqual({ type: 'docker-compose', service: 'api' });
  });
  ...
  test('includes shared services in shared config', () => {
    const config = presets.docker.generate(makeDetection({ sharedServices: ['postgres', 'redis'] }));
  ...
  test('uses detection projectName', () => {
    const config = presets.docker.generate(makeDetection({ projectName: 'cool-project' }));
  ...
  test('includes worktrees prefix', () => {
    expect(presets.docker.generate(makeDetection()).worktrees?.prefix).toBe('grove');
  });
});
...
describe('presets.vite.generate', () => {
  test('creates node-scripts provider with dev command', () => {
    ...
    expect(config.providers.web).toEqual({ type: 'node-scripts', command: 'dev' });
  });
  ...
  test('uses detection projectName', () => {
    const config = presets.vite.generate(makeDetection({ projectName: 'vite-app' }));
  ...
  test('includes worktrees prefix', () => {
    expect(presets.vite.generate(makeDetection()).worktrees?.prefix).toBe('grove');
  });
});
...
describe('presets.node.generate', () => {
  ...
  test('uses detection projectName', () => {
    const config = presets.node.generate(makeDetection({ projectName: 'my-node-app' }));
  ...
  test('includes worktrees prefix', () => {
    expect(presets.node.generate(makeDetection()).worktrees?.prefix).toBe('grove');
  });
});
