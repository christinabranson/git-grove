...
async function writeCompose(content: string, filename = 'docker-compose.yml'): Promise<void> {
  await writeFile(join(projectDir, filename), content, 'utf-8');
}

async function writePackageJson(data: object): Promise<void> {
  await writeFile(join(projectDir, 'package.json'), JSON.stringify(data), 'utf-8');
}
