...
function dockerCell(worktree: Worktree): string {
  ...
  switch (state) {
    case 'running': return chalk.green('● running') + (urls ? `  ${urls}` : '');
    case 'partial': return chalk.yellow('◐ partial') + (urls ? `  ${urls}` : '');
    case 'stopped': return chalk.gray('■ stopped');
    case 'not started': return chalk.gray('-');
  }
}
