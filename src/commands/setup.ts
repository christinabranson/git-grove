...
function printDetectionSummary(repoPath: string, detection: Awaited<ReturnType<typeof detectProject>>): void {
  console.log(chalk.bold.cyan('\nDetecting project type…\n'));

  if (detection.hasDockerCompose) {
    console.log(chalk.green('  ✔') + ' Found docker-compose.yml');
    if (detection.appServices.length > 0) {
      console.log(chalk.green('  ✔') + ` App services: ${chalk.white(detection.appServices.join(', '))}`);
    }
    if (detection.sharedServices.length > 0) {
      console.log(chalk.green('  ✔') + ` Shared infrastructure: ${chalk.gray(detection.sharedServices.join(', '))}`);
    }
  }
  ...
  if (detection.hasPackageJson) {
    ...
    console.log(chalk.green('  ✔') + ` Found package.json — framework: ${frameworkLabel}`);
  }

  if (!detection.hasDockerCompose && !detection.hasPackageJson) {
    console.log(chalk.yellow('  ⚠') + ' No docker-compose.yml or package.json found');
...
function printProposedConfig(config: GroveConfig): void {
  console.log(chalk.bold('Proposed') + ' ' + chalk.cyan('.grove/config.json') + chalk.bold(':') + '\n');
...
function printGitignoreSuggestion(): void {
  console.log(
    chalk.gray('  Add these to .gitignore (per-worktree, should not be committed):'),
  );
...
async function writeGroveConfig(repoPath: string, config: GroveConfig): Promise<void> {
...
export async function runSetup(repoPath: string, opts: SetupOptions = {}): Promise<void> {
  ...
  if (existsSync(configPath) && !opts.reset && !opts.dryRun) {
    ...
    console.log(chalk.gray('  Run with --reset to regenerate it, or --dry-run to preview a new config.'));
    return;
  }
  ...
  if (!opts.preset) {
    console.log(chalk.gray(`  Using preset: ${chalk.white(presetName)} (auto-detected)\n`));
  }
  ...
  if (!opts.yes) {
    const go = await confirm(chalk.bold('Write .grove/config.json?') + chalk.gray(' (y/N) '));
    if (!go) {
  ...
  console.log(chalk.gray('  grove spin <branch>   — create a worktree and start its environment'));
