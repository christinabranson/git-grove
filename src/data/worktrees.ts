...
async function readChangeFootprint(worktreePath: string): Promise<ChangeFootprint | null> {
  try {
    const { stdout: statOut } = await execa(
      'git',
      ['diff', '--stat', 'HEAD'],
      { cwd: worktreePath }
    );
...
async function readBaseBranch(worktreePath: string, branch: string): Promise<string | null> {
  ...
  try {
    const { stdout } = await execa('git', ['rev-parse', '--abbrev-ref', `${branch}@{upstream}`], {
      cwd: worktreePath,
    });
...
async function readDockerInfo(worktreePath: string) {
  ...
  const projectName = env['COMPOSE_PROJECT_NAME'] || path.basename(worktreePath);
  const webPort = env['WEB_PORT'] ? parseInt(env['WEB_PORT'], 10) : undefined;
  const localstackPort = env['LOCALSTACK_PORT'] ? parseInt(env['LOCALSTACK_PORT'], 10) : undefined;
  ...
  try {
    ...
    let state: 'running' | 'partial' | 'stopped' | 'not started' = 'not started';
...
async function fetchPRsByBranch(
  ...
  try {
    ...
    if (isNotFound) {
      return { prs: null, warning: 'GitHub CLI (gh) not found — install it to see PR data' };
    }
    return { prs: null, warning: 'gh not authenticated — run gh auth login to see PR data' };
  }

  let currentUser: string | null = null;
  try {
    const { stdout } = await execa('gh', ['api', 'user', '--jq', '.login'], { cwd: repoPath });
  ...
  try {
    const { stdout } = await execa(
      'gh',
      ['pr', 'list', '--json', 'number,title,url,state,headRefName,author,reviews,reviewRequests', '--limit', '100'],
      { cwd: repoPath },
    );

    const raw: GhPRJson[] = JSON.parse(stdout);
    const stateMap: Record<string, PRInfo['state']> = { OPEN: 'open', CLOSED: 'closed', MERGED: 'merged' };
    ...
    for (const pr of raw) {
      map.set(pr.headRefName, {
        ...
        youCommented: currentUser != null && pr.reviews.some((r) => r.author.login === currentUser),
      });
    }
...
export async function loadWorktrees(repoPath: string): Promise<LoadWorktreesResult> {
 ...
 */
export function resolveWorktreeRoot(repoPath: string, configRoot?: string): string {
