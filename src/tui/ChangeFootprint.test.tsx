...
function makeFootprint(overrides: Partial<ChangeFootprint> = {}): ChangeFootprint {
  return {
    byDir: {},
    totalFiles: 0,
    ...overrides,
...
describe('ChangeFootprintPanel', () => {
  test('renders directory names', () => {
    ...
    const { lastFrame } = render(
      <ChangeFootprintPanel footprint={footprint} expanded={false} width={80} />
    );
  ...
  test('expanded: shows only filename, not full path', () => {
    const footprint = makeFootprint({
      byDir: {
        'src/deep/nested': [{ path: 'src/deep/nested/file.ts', added: 5, removed: 0 }],
      },
    });
    const { lastFrame } = render(
      <ChangeFootprintPanel footprint={footprint} expanded={true} width={80} />
  ...
  test('renders empty footprint without error', () => {
    const { lastFrame } = render(
      <ChangeFootprintPanel footprint={makeFootprint()} expanded={false} width={80} />
    );
...
describe('CompactFootprint', () => {
  ...
  test('shows directory names', () => {
    ...
    const { lastFrame } = render(<CompactFootprint footprint={footprint} width={60} />);
    expect(lastFrame()).toContain('src/auth/');
  ...
  test('renders empty footprint without error', () => {
    const { lastFrame } = render(<CompactFootprint footprint={makeFootprint()} width={60} />);
    expect(lastFrame()).toContain('change footprint');
  });
});
