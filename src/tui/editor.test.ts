...
describe('resolveEditor', () => {
  ...
  test('resolves editor found on PATH via which', async () => {
    mockedExeca.mockImplementation(async (_cmd: string, args: string[]) => {
      if (args[0] === 'code') return { stdout: '/usr/local/bin/code' } as ReturnType<typeof execa>;
  ...
  test('resolves vim when it is first found on PATH', async () => {
    mockedExeca.mockImplementation(async (_cmd: string, args: string[]) => {
      if (args[0] === 'vim') return { stdout: '/usr/bin/vim' } as ReturnType<typeof execa>;
      throw new Error('not found');
    });
  ...
  test('prioritizes $VISUAL over other editors', async () => {
    ...
    mockedExeca.mockResolvedValue({ stdout: '/usr/local/bin/my-editor' } as ReturnType<typeof execa>);
  ...
  test('uses $EDITOR when $VISUAL is not set', async () => {
    ...
    mockedExeca.mockImplementation(async (_cmd: string, args: string[]) => {
      if (args[0] === 'nano') return { stdout: '/bin/nano' } as ReturnType<typeof execa>;
      throw new Error('not found');
    });
  ...
  test('uses grove config editor when provided, ignoring env vars', async () => {
    ...
    mockedExeca.mockImplementation(async (_cmd: string, args: string[]) => {
      if (args[0] === 'cursor') return { stdout: '/usr/local/bin/cursor' } as ReturnType<typeof execa>;
      throw new Error('not found');
    });
