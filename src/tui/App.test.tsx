...
describe('App — initial render', () => {
  test('shows "grove" brand in header', () => {
    const { lastFrame } = render(<App repoPath="/repo" initialWorktrees={[makeWorktree()]} />);
    expect(lastFrame()).toContain('grove');
  });

  test('shows worktree count in header', () => {
    const { lastFrame } = render(<App repoPath="/repo" initialWorktrees={[makeWorktree(), featureWorktree]} />);
    expect(lastFrame()).toContain('2 worktrees');
  });

  test('shows worktree branch names', () => {
    const { lastFrame } = render(<App repoPath="/repo" initialWorktrees={[makeWorktree({ branch: 'main' })]} />);
    expect(lastFrame()).toContain('main');
  });

  test('renders with empty worktrees list', () => {
    const { lastFrame } = render(<App repoPath="/repo" initialWorktrees={[]} />);
...
describe('App — navigation', () => {
  ...
  test('j key moves selection down to second worktree', async () => {
    const { lastFrame, stdin } = render(<App repoPath="/repo" initialWorktrees={twoWorktrees} />);
    expect(lastFrame()).toContain('(main)');
    stdin.write('j');
    await wait();
...
describe('App — footprint expand', () => {
  test('pressing x toggles footprint expansion on and off', async () => {
    ...
    const { lastFrame, stdin } = render(<App repoPath="/repo" initialWorktrees={[worktree]} />);
  ...
  test('navigating collapses footprint', async () => {
    const withFootprint = makeWorktree({
      changeFootprint: { byDir: { src: [{ path: 'src/f.ts', added: 10, removed: 3 }] }, totalFiles: 1 },
    });
    const { lastFrame, stdin } = render(<App repoPath="/repo" initialWorktrees={[withFootprint, featureWorktree]} />);
    stdin.write('x');
    await wait();
    expect(lastFrame()).toContain('-3');
    stdin.write('j');
...
describe('App — sync', () => {
  test('pressing s triggers a sync and shows a status message', async () => {
    ...
    const { lastFrame, stdin } = render(<App repoPath="/repo" initialWorktrees={[makeWorktree()]} />);
    stdin.write('s');
    await wait(100);
...
describe('App — help view', () => {
  ...
  test('pressing ? again on help screen returns to main view', async () => {
    ...
    stdin.write('?');
    await wait();
    expect(lastFrame()).not.toContain('Grove — Keyboard Shortcuts');
    expect(lastFrame()).toContain('grove');
  });
});
