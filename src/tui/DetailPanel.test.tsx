...
function renderPanel(worktree: Worktree | null, opts: { footprintExpanded?: boolean } = {}) {
  return render(
    <DetailPanel
      worktree={worktree}
      width={60}
...
describe('DetailPanel — branch header', () => {
  ...
  test('shows "(main)" label for main worktree', () => {
    const { lastFrame } = renderPanel(makeWorktree({ branch: 'main', isMain: true }));
...
describe('DetailPanel — docker section', () => {
  ...
  test('shows docker section header when docker is present', () => {
    const wt = makeWorktree({ docker: { state: 'running', projectName: 'myapp' } });
    const { lastFrame } = renderPanel(wt);
  ...
  test('shows web port URL when webPort is present', () => {
    const wt = makeWorktree({ docker: { state: 'running', projectName: 'myapp', webPort: 3000 } });
    const { lastFrame } = renderPanel(wt);
  ...
  test('shows db schema when present', () => {
    const wt = makeWorktree({
      docker: { state: 'running', projectName: 'myapp', dbSchema: 'myapp_feature' },
    });
    const { lastFrame } = renderPanel(wt);
  ...
  test('shows stopped state with ■ symbol', () => {
    const wt = makeWorktree({ docker: { state: 'stopped', projectName: 'myapp' } });
    const { lastFrame } = renderPanel(wt);
    expect(lastFrame()).toContain('stopped');
  });

  test('shows partial state', () => {
    const wt = makeWorktree({ docker: { state: 'partial', projectName: 'myapp' } });
    const { lastFrame } = renderPanel(wt);
...
describe('DetailPanel — PR section', () => {
  ...
  test('shows PR section header', () => {
    const wt = makeWorktree({
      pr: { number: 42, title: 'Add auth', url: '', state: 'open', approvals: 0, reviewRequested: false, youCommented: false, isAuthor: true },
    });
    const { lastFrame } = renderPanel(wt);
  ...
  test('shows PR number', () => {
    const wt = makeWorktree({
      pr: { number: 99, title: 'Fix bug', url: '', state: 'open', approvals: 0, reviewRequested: false, youCommented: false, isAuthor: true },
    });
    const { lastFrame } = renderPanel(wt);
  ...
  test('shows PR title', () => {
    const wt = makeWorktree({
      pr: { number: 1, title: 'Implement feature flags', url: '', state: 'open', approvals: 0, reviewRequested: false, youCommented: false, isAuthor: true },
    });
    const { lastFrame } = renderPanel(wt);
  ...
  test('shows approval count when approvals > 0', () => {
    const wt = makeWorktree({
      pr: { number: 1, title: 'My PR', url: '', state: 'open', approvals: 2, reviewRequested: false, youCommented: false, isAuthor: true },
    });
    const { lastFrame } = renderPanel(wt);
  ...
  test('shows singular "approval" for 1 approval', () => {
    const wt = makeWorktree({
      pr: { number: 1, title: 'My PR', url: '', state: 'open', approvals: 1, reviewRequested: false, youCommented: false, isAuthor: true },
    });
    const { lastFrame } = renderPanel(wt);
  ...
  test('does not show approval count when approvals is 0', () => {
    const wt = makeWorktree({
      pr: { number: 1, title: 'My PR', url: '', state: 'open', approvals: 0, reviewRequested: false, youCommented: false, isAuthor: true },
    });
    const { lastFrame } = renderPanel(wt);
  ...
  test('shows "review requested" when reviewRequested is true', () => {
    const wt = makeWorktree({
      pr: { number: 1, title: 'My PR', url: '', state: 'open', approvals: 0, reviewRequested: true, youCommented: false, isAuthor: true },
    });
    const { lastFrame } = renderPanel(wt);
...
describe('DetailPanel — change footprint section', () => {
  ...
  test('passes footprintExpanded to ChangeFootprintPanel', () => {
    ...
    const { lastFrame: expanded } = renderPanel(wt, { footprintExpanded: true });
    const { lastFrame: collapsed } = renderPanel(wt, { footprintExpanded: false });
